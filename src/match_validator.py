...
class MatchValidator:
    ...
    def titles_equal_fuzzy(self, t1: str, t2: str) -> bool:
        return self.titles_equal_normalized(t1.split(':')[0], t2.split(':')[0]) or \
                edit_distance.SequenceMatcher(a=self.normalize(t1), b=self.normalize(t2)).distance() <= 2
        ...
        normalized = ''.join(
            filter(
                ...
                unicodedata.normalize(
                    'NFKD',
                    re.sub(
                        r'( \([0-9]{4}\))', '', html.unescape(
                            self.romanize(s)).casefold().replace('&', 'and'))).strip()))
    ...
    def validate(
            ...
            title: str,
            platforms: List[str],
            aliases: List[str] = []) -> ValidationInfo:
        platform_equal = self.verify_platform(game.platform, platforms)
